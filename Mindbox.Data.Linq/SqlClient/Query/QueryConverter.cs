...
#pragma warning restore 618
        ...
        private SqlStatement VisitInsert(Expression item, LambdaExpression resultSelector) {
            ...
            var shouldOverrideAutogeneratedIdentity = false;
            foreach (var metaMember in itemMetaType.PersistentDataMembers)
            {
	            if (metaMember is AttributedMetaDataMember attributedMetaMember &&
	                attributedMetaMember.IdentityInsertColumnIdentifier != null &&
	                metaTable.Model is MindboxMetaModel mindboxMetaModel &&
	                mindboxMetaModel.ShouldOverrideAutogeneratedIdentity(attributedMetaMember
		                .IdentityInsertColumnIdentifier))
	            {
		            shouldOverrideAutogeneratedIdentity = true;
	            }

	            if (!metaMember.IsAssociation &&
	                (!metaMember.IsDbGenerated || shouldOverrideAutogeneratedIdentity) &&
            ...
            SqlInsert sin = new SqlInsert(tab, sqlItem, item, shouldOverrideAutogeneratedIdentity);
